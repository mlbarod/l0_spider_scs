...
| Self Equipment 조회 | [4. 자설비 이상감지](USER_MANUAL.md#4-자설비-이상감지), `03`~`07` 이미지 | `/self-equipment`, alias `/fdc_trend/self-equipment` | [self-equipment.md](../features/self-equipment.md) | `DF-SELF-01` in [data-flow.md](../system/data-flow.md) | `/api/self-equipment-data`, `/api/erd-scatter-data`; 별도 JSON Schema 없음 | [파일 누락·권한](../operations/troubleshooting.md#8-appdata-파일-누락-또는-권한-오류) |
...
| 공통부 이상감지 개발 예정 확인 | [6.2 공통부 이상감지](USER_MANUAL.md#62-공통부-이상감지) | `/common-anomaly`, alias `/fdc_trend/common-anomaly` | Under Construction | 해당 없음 | 해당 없음 | 해당 없음 |
...
| MY EQP 등록 개발 예정 확인 | [5. MY EQP 등록](USER_MANUAL.md#5-my-eqp-등록) | `/registration`; alias `/recipients` | Under Construction | 해당 없음 | 해당 없음 | 해당 없음 |
...
개별 STEP HMAC 링크는 현재 구현 기능이 아니다. 사용자에게 안내 가능한 직접 링크는 일반 Dashboard 상세 URL이다.
...
| 공통부 이상감지 | 공통부 이상감지 | `/under-construction/common-anomaly` | `/common-anomaly`, `/fdc_trend/common-anomaly` | `개발예정` | `routes.jsx`; `L0SpiderHomePage.jsx` |
...
| MY EQP 등록 | MY EQP 등록 | `/under-construction/my-eqp-registration` | `/registration`, `/recipients` 및 `/fdc_trend/...` | `개발예정` | `routes.jsx`; `L0SpiderHomePage.jsx` |
...
| `images/03-self-equipment-filters.png` | PNG 1920×1080 | Self Equipment 필터 | 4.1에 직접 삽입 | 현재 3일 동일성 toggle과 RECIPE_ID label을 반영하지 않아 `Mismatch` |
...
|---|---|---|---|---|
| `MAN-M01` | `02-main-menu.png`의 메뉴명·운영 상태 | 현재 메인에는 `MY EQP 등록` 개발예정 카드가 있음 | 오래된 카드명으로 메뉴를 찾지 못할 수 있음 | 현재 `L0SpiderHomePage.jsx`의 메뉴명을 사용 |
