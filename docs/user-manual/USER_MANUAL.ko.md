...
> 기준일: 2026-08-30
> 현재 제공 기능: Dashboard, 자설비 이상감지, 동일성 이상감지, 사용자 매뉴얼
...
| 상태 | 기능 |
|---|---|
| 운영중 | 자설비 이상감지, 동일성 이상감지, 사용자 매뉴얼 |
| 개발예정 | 공통부 이상감지, 공통부 동일성 이상감지, FDC Hard Limit 추천, MY EQP 등록, Defect SPIDER, L1 SPIDER, L3 SPIDER |

메인 화면 아래의 `라인별 이상감지 대시보드`는 운영중입니다. `개발예정` 카드를 누르면 기능 화면 대신 준비 중 안내가 표시됩니다.
...
5. `라인별 이상 현황`, `라인별 일간 이상 추이`, `라인 상세`를 확인합니다.

`초기화`는 조회 조건을 기본값으로 되돌립니다. `다시 시도`는 같은 조건으로 서버 데이터를 다시 불러옵니다. 상세 표의 열 제목을 누르면 정렬할 수 있습니다.
...
앞 단계 선택에 따라 다음 목록이 달라집니다. 원하는 값이 없으면 앞 단계 조건, 데이터 기준일 또는 Sensor 제외 설정을 확인합니다. `다시 시도`는 현재 조건의 데이터를 새로 불러옵니다.

### 3.2 차트 확인

필터 선택이 완료되면 `Scatter 차트` 영역에 EQP별 차트가 표시됩니다.

- `ch_step 모아보기`: 같은 EQP의 여러 `ch_step`을 한 영역에서 봅니다.
- `전체 ch_step 보기`: 모아보기 상태를 원래 개별 표시로 되돌립니다.
- `동일성 차트`: 선택 EQP의 동일성 데이터를 별도 창에서 봅니다.
- `3일 동일성 차트 표시`: 모아보기 오른쪽에 최근 72시간 동일성 차트를 함께 표시합니다.
- `변경점 이력`: EQP 작업 이력과 연결 가능한 LINK를 확인합니다.

차트 데이터가 없다는 안내는 조회 자체가 실패했다는 뜻이 아닐 수 있습니다. 선택 조건에 맞는 원천 데이터가 없는지 먼저 확인합니다.

### 3.3 SKIP 작업

`SKIP`, `EQP ALL SKIP`, `SKIP 해제`는 DB capability가 활성이고 사용자 정보가 준비된 경우에만 사용할 수 있습니다.
...
결과가 많으면 페이지 버튼으로 이동합니다. `다시 시도`는 최신 경로와 현재 조건을 다시 확인합니다.
...
| 화면 메시지·증상 | 확인할 내용 |
|---|---|
| `선택한 필터와 일치하는 데이터가 없습니다.` | 앞 단계 필터와 데이터 기준일을 확인합니다. |
| 데이터를 불러오지 못함 | 잠시 후 `다시 시도`하고 계속되면 URL·시각·선택 조건을 운영자에게 전달합니다. |
