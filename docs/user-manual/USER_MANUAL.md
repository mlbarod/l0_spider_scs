...
별도 로그인 화면은 없습니다. 이력 기능은 서버가 확인한 접속 IP를 `knox_id` 컬럼에 기록합니다.
My EQP·Mailing 등록 화면의 `knox_id`는 자동 결정하지 않으므로 수신인 ID를 직접 입력해야 합니다.
자설비 화면은 별도 DB 식별 계약이 준비되지 않아 오른쪽 위에 DB 기능 비활성 안내를 계속 표시합니다.
...
| My EQP 등록 | 메일 수신 조건과 모니터링 설비·열람자 등록 |
...
- 동일성·공통부의 DB 기능이 활성인 기존 흐름에서는 마지막 필터 선택 이력을 저장할 수 있습니다.
  현재 자설비 file-only 화면은 클릭이력을 저장하지 않습니다.

클릭이력과 결과 이력은 현재 자설비 화면에서 모두 비활성입니다.
...
아래 SKIP·이력저장 버튼 설명은 기존 DB 계약의 참고용입니다. 현재 SCS `path_xian` 화면에는
표시되지 않습니다.
...
현재 SCS `path_xian` 배포에서는 **동일성 차트**와 **변경점이력**만 표시됩니다. 새 index에
기존 Self DB 식별자 `ver`가 없으므로 전역 data gate가 활성화되어도 SKIP 계열과 이력저장은
표시되지 않습니다. 재활성화에는 새 DB 식별 계약이 필요합니다.

SKIP 사유·해제 절차는 legacy DB 계약 참고용이며 현재 SCS 자설비 화면에서는 사용할 수
없습니다. `SKIP LIST`와 **SKIP해제**도 새 DB 식별 계약이 정의될 때까지 표시되지 않습니다.

![SKIP 확인](images/06-self-equipment-skip-dialog.png)

## 5. My EQP 등록

메인 화면의 **My EQP 등록**에서 두 기능을 함께 관리합니다. 처음에는 두 영역이 모두 접혀 있으며 **펼치기/접기**로 필요한 영역만 표시합니다.
...
Mailing 또는 My EQP 중 한 영역만 완성해도 저장할 수 있으며, 둘 다 완성하면 함께 처리됩니다.
My EQP 조건은 지정한 `knox_id`별로 등록되지만 현재 SCS 자설비 화면의 `MY EQP` 조회에는
연결되지 않습니다.
...
신규 등록은 입력한 `knox_id`에게만 공개됩니다. 만료된 조건은 DB에서 자동 삭제되지 않습니다.
현재 자설비 화면에는 `MY EQP` 선택지가 없으므로 이 등록 목록에서 자설비 조회로 이어지는
절차는 `Blocked`입니다.
...
두 Report의 이상건수는 대시보드와 같은 고유 이상건 조합 기준입니다. 전체설비 Report와 My EQP Report는 메일을 받는 `knox_id`의 등록 조건만 각각 표시합니다. 한쪽에만 등록했다면 다른 Report는 빈 표로 표시됩니다. 일반 LINK로 자설비 화면에 접속하면 URL에 포함된 Line·SDWT·Grade가 자동 선택되며, A 또는 B Grade는 화면의 `A/B` 필터로 적용됩니다. template의 My EQP LINK는 `sdwt=MY_EQP`, `step=ALL`, `eqpCh`를 포함하지만 현재 자설비의 `MY EQP` capability가 비활성이라 화면 조회 연결은 `Blocked`입니다.
...
legacy 계약에서는 **MY EQP** 선택 시 STEP **ALL**을 사용하지만 현재 자설비 화면에는 해당
선택지가 표시되지 않습니다.
...
| 작업 | 영향 |
| --- | --- |
| 공통부 SKIP / SKIP해제 | 공통부 이상건을 활성 결과에서 제외하거나 해제 |
| 비-Self 이력저장 | 동일성·공통부·공통부 동일성의 현재 카드 결과를 이력으로 저장 |
| 비-Self 조회의 최종 필터 선택 | 조회 카테고리 이력을 자동 저장. sensor `ALL`은 `ALL`로 기록 |
...
Self의 SKIP·EQP ALL SKIP·SKIP해제·이력저장·클릭이력은 현재 화면에서 미노출·미호출입니다.
...
| 비-Self 클릭이력 저장 실패 | 접속 IP, DB 연결과 `clicked_category_history` 저장 상태 확인 |
| 접속 IP를 확인할 수 없습니다. | proxy forwarding header와 socket 주소 확인 |
