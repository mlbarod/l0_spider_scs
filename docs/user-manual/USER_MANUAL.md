...
SCS의 MY EQP 기능은 아직 제공되지 않습니다. 메인 화면의 **MY EQP 등록** 카드는
`개발예정` 상태이며 Under Construction 안내 화면으로 이동합니다.
...
| MY EQP 등록 | 개발 예정 안내 화면 확인 |
| 동일성 이상감지 | 동일 조건의 분석 완료 이미지 비교 |
| 공통부 이상감지 | 개발 예정 안내 화면 확인 |
| 사용자 메뉴얼 | 현재 문서 확인 |
...
메인 화면의 **MY EQP 등록** 카드는 현재 `개발예정` 상태입니다.
카드를 선택하거나 `/registration`, `/recipients`로 직접 접속하면 Under Construction 안내 화면이 표시됩니다.
...
메인 화면의 **공통부 이상감지** 카드는 현재 `개발예정` 상태입니다.
카드를 선택하거나 `/common-anomaly`로 직접 접속하면 Under Construction 안내 화면이 표시됩니다.
...
| 작업 | 영향 |
| --- | --- |
| 자설비 SKIP / SKIP해제 | 이상건을 활성 결과에서 제외하거나 해제 |
| 이력저장 | 현재 운영 중인 이상감지 카드 결과를 이력으로 저장 |
| 조회의 최종 필터 선택 | 조회 카테고리 이력을 자동 저장. sensor `ALL`은 `ALL`로 기록 |
| MY EQP 등록 | 개발예정 상태로 현재 데이터 변경 없음 |
