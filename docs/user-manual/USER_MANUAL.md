...
별도 로그인 화면은 없습니다. DB credential이 연결된 배포에서는 등록 화면의 현재 사용자와
My EQP·Mailing 등록 기능을 사용할 수 있습니다. 자설비 화면은 별도 식별 계약이 준비되지 않아
오른쪽 위에 사용자명 대신 DB 기능 비활성 안내를 계속 표시합니다.
...
`path/{latest_date}`의 `{latest_date}`를 표시합니다. 이 표시는 전체 Dashboard 집계와 분리된
최신 시각 조회를 사용하므로 통계·매핑 집계 상태에 의존하지 않습니다.
...
| 비-Self 클릭이력 저장 실패 | 접속자 확인, DB 연결과 `clicked_category_history` 저장 상태 확인 |
