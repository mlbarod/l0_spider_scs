...
DB credential이 준비된 환경에서는 My EQP, SKIP LIST, SKIP, 클릭이력과
...
별도 로그인 화면은 없습니다. SCS 서버는 `knox_id`를 조회하지 않고 접속 IP를 My EQP와
이력 테이블의 사용자 식별값으로 사용합니다. 기존 DB 컬럼명은 `knox_id`지만 저장값은 IP입니다.
My EQP 등록은 현재 접속 IP로 자동 저장되고, Mailing 등록의 수신인 `knox_id` 입력은 별도 기능입니다.
...
- 자설비의 MY EQP 진입은 한 번의 가상 카테고리 이력을 기록하며 ch_step 변경에서 중복 저장하지 않습니다.
...
4. **등록 사용자** 영역에서 현재 접속 IP 자동 등록 안내를 확인합니다.
5. 화면 아래의 **저장 및 Mailing등록**을 누릅니다.

Mailing 또는 My EQP 중 한 영역만 완성해도 저장할 수 있으며, 둘 다 완성하면 함께 처리됩니다.
My EQP 조건은 현재 접속 IP로 등록되며, 같은 IP로 자설비 화면에 접속하면
SDWT 선택 목록의 **MY EQP**에서 활성 등록 EQP를 조회할 수 있습니다.
...
- 내가 등록한 개인·공개 조건
- 내 접속 IP에 등록된 조건과 과거 전체 공개 조건
- 모니터링 상태, 등록·만료 시각, 공개 여부, EQP와 Comment

다른 사용자의 공개 조건은 조회만 가능하며 삭제할 수 없습니다. 본인 등록 건만 **삭제** 버튼이 표시됩니다.
신규 등록은 현재 접속 IP에만 연결됩니다. 만료된 조건은 DB에서 자동 삭제되지 않습니다.
활성 등록이 하나 이상 있으면 자설비 화면에 **MY EQP** 선택지가 표시됩니다.
...
두 Report의 이상건수는 대시보드와 같은 고유 이상건 조합 기준입니다. 전체설비 Report는 입력한 수신인 `knox_id`, SCS My EQP 조회는 접속 IP의 등록 조건을 사용합니다. IP 기반 My EQP 등록을 실제 메일 주소로 변환하는 발송기 계약은 `Unknown`입니다. 일반 LINK로 자설비 화면에 접속하면 URL에 포함된 Line·SDWT·Grade가 자동 선택되며, A 또는 B Grade는 화면의 `A/B` 필터로 적용됩니다. template의 My EQP LINK는 `sdwt=MY_EQP`, `step=ALL`, `eqpCh`를 포함하고, 활성 등록과 DB capability가 있으면 MY EQP 조회로 연결됩니다.
...
SCS My EQP는 다른 사용자를 직접 지정하지 않습니다. 상대방 PC에서 접속해 해당 접속 IP로 등록해야 합니다.
