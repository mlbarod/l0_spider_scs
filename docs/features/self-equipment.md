...
`ver`가 있는 활성 SKIP LIST row는 `pathSdwt=__SKIP_LIST__`를 chart 요청에 전달한다. 서버는
기존 `l0_spider`와 같이 row의 ERD `file_path`를 그대로 받아 같은 디렉터리의 `data.parquet`을
Scatter·동일성 chart에서 공통으로 읽는다.
...
| 같은 API | `line`, `pathSdwt` | query | string | 일반 row 필수 | 일반 row는 분임조별 path table 재검증; SKIP LIST는 `__SKIP_LIST__` sentinel로 ERD 경로 직접 읽기 | chart row | `Confirmed` |
| 같은 API | `sensor`, `chStep`, `ver`, `latestDate` | query | string | 일반 row는 sensor·chStep·ver 필수, latestDate 선택 | row 값 사용; latestDate 형식 검증 | chart row | 코드 `Confirmed` |
