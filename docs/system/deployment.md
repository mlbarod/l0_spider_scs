...
| `SCS_DB_CONNECTIONS_ENABLED` | credential 기반 DB 전용 API allowlist | 활성 후보 | 전체 gate 비활성 mode에서 `DB_INFO_PATH`가 읽기 가능해야 활성; allowlist 차단 또는 UI shell은 `0` |
...
4. runtime 환경변수 이름만 대조하고 실제 값은 출력하지 않는다.
   - 기본 부분 연결은 Dashboard·Self gate가 미설정 또는 `1`이고 전체 gate는 비활성인지 확인한다. DB는 credential read 가능 여부와 DB gate를 확인한다. UI shell은 세 범위 gate가 비-`1`인지 값 노출 없이 확인한다.
...
8. 기본 부분 연결이면 Dashboard GET/HEAD와 mapping·Self·scatter GET이 통과하고 다른 App은 503인지 확인한다. 읽기 가능한 credential이 있으면 승인된 DB 전용 API도 handler에 진입한다.
