...
1. `SCS_DATA_CONNECTIONS_ENABLED=1`이면 전체 API가 활성화된다. 그렇지 않은 상태에서는 기본적으로 자설비 file read allowlist만 통과하고 나머지 `/api` 요청은 `503 DATA_CONNECTIONS_DISABLED`로 종료된다. `SCS_SELF_EQUIPMENT_DATA_ENABLED=0` 등 비-`1` 값을 명시하면 자설비 allowlist도 차단된다.
...
- 현재 코드는 기본적으로 자설비 파일 read allowlist만 활성화하고 다른 App과 DB는 계속 차단한다. 전체 UI shell은 `SCS_SELF_EQUIPMENT_DATA_ENABLED=0` 같은 비-`1` 값을 명시해 전환한다. 실제 배포 환경의 변수 존재·값과 target mount는 `Unknown`이다.
