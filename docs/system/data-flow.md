...
| `DF-SELF-01` | Self Equipment 필터 | `/self-equipment` | mapping·최신 path_xian row·종속 필터 | `path_xian/{latest_date}`; DB history 미결합 | REICPE_ID·EQP·sensor·ch_step·chart row | 코드 `Complete`; 운영 `Unknown` | `Confirmed`/`Unknown` |
...
| `DF-SELF-01` | `GET /api/self-equipment-data` | `readLatestSelfEquipmentRows`, `scopeSelfEquipmentRows`, `buildSelfEquipmentPayload` | `DS-SELF-01`; DB history 미결합 | mapping 범위, REICPE_ID=`REICPE_ID`, ch_step=`step`, 종속 option·row 생성 | 코드 `Confirmed`; 운영 `Unknown` | `selfEquipmentData.mjs` |
...
| `REICPE_ID` / legacy `desc` | 사용자 | UI label은 REICPE_ID; API wire 이름은 legacy `desc` | path_xian Parquet `REICPE_ID` filter | URL `step`과 직접 연결 안 함 | 코드 `Confirmed` / URL 명칭 `Mismatch` | `FdcTrendPage`; self handler |
...
|---|---|---|---|---|---|---|
| 1 | `FdcTrendPage` | mapping, 사용자 선택 | Line→SDWT→Grade→REICPE_ID→EQP→sensor→ch_step 상태 구성 | API query | `Confirmed` | page query state |
