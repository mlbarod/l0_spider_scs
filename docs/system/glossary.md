...
| STEP | 업무 용어 | 설비 이상 결과를 공정 STEP 단위로 선택하는 화면 개념 | UI `STEP`, Self 원천 row `step`, 호환 응답 `desc`, query `step` | Self Equipment, 동일성, 딥링크 | 현재 Self는 `path_xian.step`을 STEP과 `ch_step` 양쪽에 사용한다. URL `step`의 비-`ALL` token 연결은 별도 후보다. | Self 원천 사용 `Confirmed`; 비-`ALL` 딥링크 연결 `Mismatch` — `self-equipment.md:224-233` |
...
| `ch_step` | 업무·화면 용어 | 선택 sensor 아래의 channel STEP 차원 | UI `ch_step`, API `chStep`, Self row `step` | Self Equipment, 동일성, 공통부 | 현재 Self에서는 공정 STEP과 같은 `path_xian.step` 값을 사용한다. point column은 `${sensor}*${chStep}`, 공통부는 `${sensor}_${chStep}` 형태다. | Self 계약 `Confirmed` — `data-flow.md`; `self-equipment.md` |
