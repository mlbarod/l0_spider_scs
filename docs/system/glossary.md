...
| RECIPE_ID | Self 화면 용어 | 자설비 이상 결과를 recipe 식별자 단위로 선택하는 상위 필터 | UI `RECIPE_ID`, Self 원천 row `recipe_id`, 호환 응답 `steps[].desc`, query `desc` | Self Equipment | 원천 `recipe_id`를 projection하고 UI에서 `RECIPE_ID`로 표시한다. | 코드 `Confirmed`; 운영 값 `Unknown` — `self-equipment.md` |
...
| `desc` | 코드 식별자 | Dashboard의 STEP 설명 또는 Self 호환 응답 필드 | `desc` | Dashboard, Self Equipment, Mailing 집계 | 현재 Self에서는 `path_xian.recipe_id`를 호환 응답 `desc`로도 투영한다. 원천 `desc` 컬럼으로 해석하지 않는다. | Self 투영 `Confirmed` — `self-equipment.md:224-249`; `data-flow.md` |
...
| `ch_step` | 업무·화면 용어 | 선택 sensor 아래의 channel STEP 차원 | UI `ch_step`, API `chStep`, Self row `step` | Self Equipment, 동일성, 공통부 | Self point column은 실제 schema의 `${sensor}_${chStep}`을 우선하고 `${sensor}*${chStep}`도 호환한다. | Self 코드 `Confirmed`, 운영 schema `Unknown` — `data-flow.md`; `self-equipment.md` |
