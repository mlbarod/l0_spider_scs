...
| SCS Self Equipment read gate | `SCS_SELF_EQUIPMENT_DATA_ENABLED` | Node·Vite 선행 middleware | 미설정 또는 `"1"`; Self·scatter GET handler 활성 | 기본 file read에 따른 source path 노출·mount read | `Implemented` / 운영값 `Unknown` | `server/dataConnections.mjs`; self read API |
...
| SCS Common Anomaly read gate | `SCS_COMMON_ANOMALY_DATA_ENABLED` | Node·Vite 선행 middleware | 미설정 또는 `"1"`; mapping과 공통부 path·scatter·image read handler 활성 | 기본 file·image read에 따른 source path 노출·mount read | `Implemented` / 운영값 `Unknown` | `server/dataConnections.mjs`; common anomaly API |
...
SCS 기본 실행에서는 Dashboard, 자설비, 동일성과 공통부 이상감지 read를 허용한다. 읽기 가능한 credential이 있으면
사용자·My EQP와 세 이력 API도 허용하지만 Mailing·다른 App은 계속 차단한다.
전체 UI shell이 필요하면 Dashboard·Self·동일성·공통부 이상감지·DB 범위 변수를 모두 `0`으로 명시한다.
...
| SCS API gate | `/api` namespace | handler dispatch 전 전체 opt-in, Dashboard·Self·동일성·공통부 이상감지 read와 credential 기반 DB method allowlist | 비허용 요청은 `503 DATA_CONNECTIONS_DISABLED`, UUID `requestId`; HEAD 무본문 | 부분 활성 상태를 전체 활성으로 오판할 위험 | `Implemented` / 운영 적용 `Unknown` | `dataConnections.mjs`; safe error contract |
...
| Self SKIP LIST data | `pathSdwt=__SKIP_LIST__`, chart row `file_path` | 활성 `pass_history`에서 ERD image path 재구성 | `/pic_server2/`→`/pic/`, ERD root prefix 검사 후 같은 directory의 `data.parquet` 직접 선택 | Parquet read | browser 경로 노출·ERD root 내부 직접 조회 | `Implemented` / 운영 `Needs Validation` | `isDirectSkipListErdPathAllowed`; `resolveErdDataFilePath` |
...
| ERD data | browser가 받은 team row `file_path` | `resolveErdDataFilePath` | `/pic_server2/`→`/pic/`, `.png` sibling·directory 하위·직접 `data.parquet` 구분, pic root prefix·backup 거부; 분임조별 row의 path·EQP·date·sensor·step·ver 재검증 후 data `ver` 없음 path-scope 또는 exact 우선·단일값 file-scope fallback·다중값 mismatch 차단 | Parquet read | symlink·producer 신뢰·화면 경로 노출 | `Implemented` / 운영 `Needs Validation` | `resolveErdDataFilePath`; `isSelfEquipmentDataPathAllowed` |
...
| MY EQP | 접속 IP가 `knox_id` 컬럼에 저장된 active registration 조회 | 개인·public 등록 범위 | proxy trust·NAT 공유 | `Risk` | `selfEquipmentData.mjs` |
...
| data connection gate | `SCS_DATA_CONNECTIONS_ENABLED` | Node·Vite | API 응답에 간접 영향 | 비활성 | 정확히 `1`이 아니면 Dashboard·Self·동일성·공통부 이상감지·DB allowlist 외 `/api` 차단 | secret 아님; owner 승인 통제 | `Implemented` / 실제 값 `Unknown` | `server/dataConnections.mjs` |
...
| Self Equipment read gate | `SCS_SELF_EQUIPMENT_DATA_ENABLED` | Node·Vite | Self·scatter GET에 직접 영향 | 활성 | 미설정 또는 정확히 `1`이면 자설비 read allowlist 통과; 그 외 값은 차단 | secret 아님; 부분 연결 통제 | `Implemented` / 실제 값 `Unknown` | `server/dataConnections.mjs` |
...
| Common Anomaly read gate | `SCS_COMMON_ANOMALY_DATA_ENABLED` | Node·Vite | mapping과 공통부 path·scatter·image read에 직접 영향 | 활성 | 미설정 또는 정확히 `1`이면 공통부 이상감지 read allowlist 통과; 그 외 값은 차단 | secret 아님; 부분 연결 통제 | `Implemented` / 실제 값 `Unknown` | `server/dataConnections.mjs` |
...
connection gate들은 비밀값은 아니지만 영향도가 큰 운영 통제다. 값을 로그에 복사하기보다
기본 부분 연결·명시적 UI shell·전체 연결 중 승인 범위만 확인한다. 전체 gate는 다른 App과
DB까지 승인된 경우에만 `1`로 설정하며, gate 없는 과거 artifact로 rollback하지 않는다.
...
|---|---|---|---|---|---|
| 인증 | 이력 식별 | 접속 IP를 이력 `knox_id` 컬럼에 기록 | `Implemented` | `Confirmed` | trusted proxy·NAT 영향 |
...
| DB query | injection·무결성 | value parameterization·transaction; My EQP runtime DDL 제거 | 일부 `Implemented` | `Confirmed` | TLS·minimum grant |
...
| Vite API surface | MY EQP·history route parity 보완 | route 수동 등록 구조 | 후속 route 누락 가능 | route parity 회귀 확인 |
...
| SEC-R03 | My EQP | `Risk` / `Needs Validation` | 조회 사용자는 DB에서 확인하지만 복수 수신자 지정 권한은 별도 role 검증 없음 | owner 경계 혼동 | `High` | 실패 정책·권한 요구 확정 |
...
| SEC-R06 | DB privilege | `Risk` / `Unknown` | read/write가 같은 credential 후보 | 과권한 blast radius | `High` | DB grant·migration 책임 확인 |
