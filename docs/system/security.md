...
| Self SKIP LIST data | `line`, `pathSdwt=__SKIP_LIST__`, chart row | 활성 `pass_history`에서 ERD image path 재구성 | Line·정규화 경로·EQP·date·sensor·step·ver exact match, 72시간 active 확인, 공통부 `ver=NA` 제외; 같은 Line의 동시 chart 요청은 5초 PASS 목록 cache 공유 | Parquet read | DB 가용성·최대 5초 stale authorization | `Implemented` / 운영 `Needs Validation` | `authorizeSelfEquipmentSkipListDataPath`; `isSelfEquipmentSkipListDataPathAllowed` |
