...
| ERD data | browser가 받은 team row `file_path` | `resolveErdDataFilePath` | `/pic_server2/`→`/pic/`, `.png` sibling·directory 하위·직접 `data.parquet` 구분, pic root prefix·backup 거부; 분임조별 row의 path·EQP·date·sensor·step·ver 재검증 후 data exact 우선·단일값 file-scope fallback·다중값 mismatch 차단 | Parquet read | symlink·producer 신뢰·화면 경로 노출 | `Implemented` / 운영 `Needs Validation` | `resolveErdDataFilePath`; `isSelfEquipmentDataPathAllowed` |
