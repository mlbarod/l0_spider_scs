...
| ERD point | path_xian row `file_path`에서 해석한 `data.parquet` | scatter·identity chart | 코드 `Confirmed`; 운영 file `Unknown` |
| ERD 변경이력 | 선택한 `data.parquet` sibling `{eqp}.parquet` | 변경점 목록 | 코드 `Confirmed`; 운영 file `Unknown` |
