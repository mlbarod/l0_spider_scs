...
- [ ] 일반 Self가 team `path_xian/{line}/{sdwt}/df_path.parquet`를 직접 읽고 row의 `ver`를 chart·SKIP에 그대로 전달하며, 단일설비 data의 선택적 `ver` 없음 path-scope·exact 우선·단일값 file-scope fallback·다중값 mismatch 차단을 확인했다.
- [ ] SKIP LIST chart가 `pathSdwt=__SKIP_LIST__`로 전달된 ERD `file_path`의 같은 디렉터리 `data.parquet`을 Scatter·3일 동일성·동일성 팝업에서 공통으로 읽는지 확인했다.
...
- [ ] DB 등록·history API의 body limit·validation과 접속 IP를 저장하는 `knox_id` 컬럼 영향을 확인했다.
- [ ] Vite 단독 mode와 통합 `server.mjs`의 MY EQP·history route parity를 확인했다.
...
- [ ] MY EQP template의 `sdwt=MY_EQP`, `step=ALL`, `eqpCh` 형식과 Self 화면 소비를 확인했다.
...
- [ ] 이번 배포가 기본 Dashboard·Self·동일성·공통부 이상감지 read 및 조건부 DB 부분 연결, 명시적 SCS UI shell, 전체 데이터 연결 중 어느 범위인지 release 기록에 명시했다.
- [ ] 기본 부분 연결이면 Dashboard·Self·동일성·공통부 이상감지 gate가 미설정 또는 `1`인지, DB credential read 가능 여부와 DB gate를 확인했으며, UI shell이면 다섯 범위 gate가 비-`1` 값인지 값 노출 없이 확인했다.
- [ ] UI shell이면 `/api`와 `/api/*`의 `503 DATA_CONNECTIONS_DISABLED`, `requestId`, `HEAD` 무본문을 확인했다.
- [ ] 기본 부분 연결이면 Dashboard GET/HEAD, mapping·Self·scatter GET, 동일성 latest path·data·image와 공통부 이상감지 path·scatter·image read가 통과하고, credential이 읽기 가능할 때 사용자·My EQP와 세 이력 API가 handler에 진입하며 Mailing·다른 App은 503이다.
...
- [ ] `step=ALL`, `eqpCh` link 형식이 Self 화면의 MY EQP 조회로 연결된다.
