...
SCS 기본 실행은 Dashboard, 자설비, 동일성과 공통부 이상감지 file read를 허용하고, 읽기 가능한 `DB_INFO_PATH`가 있으면
사용자·My EQP와 세 이력 DB API를 허용한다. 아직 연결하지 않은 다른 App의 503은 정상이다. 전체 UI shell이 필요하면
